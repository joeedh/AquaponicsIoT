...
define([
  ...
  var tokdef = exports.tokdef = class tokdef {
    constructor(name, regexpr, func, custom_reader) {
      this.name = name;
      this.re = new RegExp(regexpr, 'y');
      this.func = func;
      this.custom_reader = custom_reader;
    }
  }
  ...
  //errfunc is optional, defines state-specific error function
    ...
    next(ignore_peek) {
      ...
      var max = 55;
      var length = this.lexdata.length > this.lexpos+max ? this.lexpos+max : this.lexdata.length
      var lexdata = this.lexdata.slice(this.lexpos, this.lexpos+length);
      
      var max_res = 0;
      var theres = undefined, thet = undefined;
      
      for (var i=0; i<tlen; i++) {
        //var tstart = util.time_ms();
        
        var t = ts[i];
        
        if (t.re === undefined && t.custom_reader === undefined) {
          console.log("Warning: possible malformed token " + t.name, t);
          continue;
        }
        
        var res;
        if (t.custom_reader !== undefined) {
          console.log("custom reader");
          res = t.custom_reader(lexdata);
        } else {
          t.re.lastIndex = 0;
          res = t.re.exec(lexdata);
        }
        
        //var tend = util.time_ms();
        
        //if (tend - tstart > 0) {
        //  console.log(t.name, tend - tstart);
        //}
        
        if (res && res.index == 0 && res[0].length > max_res) {
          theres = res;
          thet = t;
      ...
      if (theres == undefined) {
        var lexlen = Math.max(this.lexdata.length, 70);
        console.log("Failed to find token match!", this.lexdata[this.lexpos], '"' + this.lexdata.slice(this.lexpos, lexlen) + '"');
        this.error();
        return;
      }
      ...
      var lexlen = max_res;
      var tok = new token(def.name, theres[0], this.lexpos, lexlen, this.lineno, this, undefined);
      this.lexpos += max_res;
  ...
  var parser = exports.parser = class parser {
    ...
    error(tok, msg) {
      ...
      var max = 65*100;
      if (buf.length > max) {
        buf = buf.slice(0, max) + "(rest is truncated...)";
      }
  ...
  function test_parser() {
    ...
    var tk = exports.tk = function tk(name, re, func) {
